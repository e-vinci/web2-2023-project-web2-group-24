...
import { checkPrivacyKey, setPrivacyKey } from '../utils/privacy';

const renderPrivacyText = () => {
  const privacy = checkPrivacyKey();
  if (privacy) {
    return;
  }
  ...
  const newModal = new Modal(document.querySelector('#privacyPolicyModal'));
  newModal.show();
...
export default renderPrivacyText;